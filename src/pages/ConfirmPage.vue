<template>
  <section class="wrap">
    ...
    <div class="card">
      ...
      <p><strong>{{ t('confirm.text') }}:</strong>
        {{ profile.nlp_text || text || t('confirm.voiceNo') }}
      </p>
      <p><strong>{{ t('confirm.voice') }}:</strong>
        {{ profile.voice_text || (audioBlob ? t('confirm.voiceYes') : t('confirm.voiceNo')) }}
      </p>
      <p v-if="profile.nlp_assessment">
        <strong>NLP:</strong>
        {{ profile.nlp_assessment }}
        <span v-if="profile.nlp_confidence != null"> ({{ profile.nlp_confidence }}%)</span>
      </p>
      <p v-if="profile.skin_analysis">
        <strong>Skin:</strong>
        {{ profile.skin_analysis }}
        <span v-if="profile.skin_confidence != null"> ({{ profile.skin_confidence }}%)</span>
      </p>
    </div>
...
async function submitSummary () {
  ...
  try {
    ...
    const next = (data.next_steps && data.next_steps.length)
      ? `\n\nNext steps:\n• ${data.next_steps.join('\n• ')}`
      : ''
    result.value = `${data.summary || ''}${next}`
  } catch (e) {
    error.value = e?.response?.data?.message || e.message || 'NetworkError when attempting to fetch resource.'
  } finally {
...
.result{white-space:pre-wrap;background:#fff;border:1px solid var(--border);padding:10px;border-radius:8px;margin:0}
</style>
