<template>
  <section class="wrap">
    ...
    <div class="back-info">
      <button class="btn outline" @click="goInfo">Modify Personal Information</button>
    </div>

    <!-- Step indicator => pager -->
    <div class="pager">
      <button :class="{on: step===1}" @click="step=1">1 Images</button>
      <button :class="{on: step===2}" @click="step=2">2 Text/Voice</button>
      <button :class="{on: step===3}" @click="step=3">3 Self-Assessment</button>
    </div>
    ...
    <div v-if="step===1" class="card">
      ...
      <div class="tags" v-if="selected.length">
        <button v-for="k in selected" :key="k" class="tag" @click="removeTag(k)">
          {{ labelOf(k) }} <span class="x">×</span>
        </button>
      </div>
      <small class="muted" v-else>Click on images to select symptoms; click × on tags to cancel selection.</small>
    ...
    <div v-else-if="step===2" class="card">
      <h3>Text / Voice</h3>
      <textarea v-model="text" rows="6" placeholder="You can enter additional descriptions (optional)"></textarea>
      <div class="actions">
...
function removeTag(k){ store.removeSymptom(k) }
...
const severityTexts = [
  '1 Very Mild: Almost no impact on daily life','2 Mild: Slight discomfort','3 Mild: Occasionally affects activities',
  '4 Mild-Moderate: Needs attention','5 Moderate: Affects daily work/study','6 Moderate-Severe: Noticeable discomfort',
  '7 Relatively Severe: Needs prompt handling','8 Severe: Recommend seeking medical attention soon','9 Very Severe: High concern','10 Critical: Seek immediate medical/emergency care',
]
const feelingTexts = [
  '1 Very Reassured','2 Fairly Reassured','3 Okay','4 A Bit Worried','5 Moderately Worried',
  '6 Noticeably Worried','7 Very Worried','8 Extremely Worried','9 Highly Anxious','10 Panicked/Urgent Help Needed',
]
function severityDesc(v){ return v ? severityTexts[v-1] : 'Drag the slider to select (1-10)' }
function feelingDesc(v){ return v ? feelingTexts[v-1] : 'Drag the slider to select (1-10)' }

// Navigation
function next(){ step.value = Math.min(3, step.value + 1) }
function skip(){ step.value = Math.min(3, step.value + 1) }
function goConfirm(){ router.push('/confirm') }
function goInfo(){ router.push('/info') } // Added: Navigate to Info page
...
.back-info{display:flex;justify-content:flex-end;margin-bottom:12px}
.back-info .btn{font-size:14px;padding:6px 14px}
...
.tag{display:inline-flex;align-items:center;gap:6px;border:1px solid var(--btn-outline-border);background:transparent;color:inherit;border-radius:999px;padding:6px 10px;cursor:pointer}
.tag .x{font-weight:700}
.row{display:flex;gap:12px;align-items:center;margin:8px 0}
.row input[type="range"]{flex:1}
.desc{color:var(--muted);margin-top:4px}
textarea{width:100%;padding:10px;border:1px solid var(--border);border-radius:10px;background:transparent;color:inherit}
</style>