...
tok = T5TokenizerFast.from_pretrained(REPO_ID, subfolder="mt_model_mt5")
dtype = torch.float16 if device == "cuda" else None
model = MT5ForConditionalGeneration.from_pretrained(REPO_ID, subfolder="mt_model_mt5", torch_dtype=dtype).to(device).eval()
