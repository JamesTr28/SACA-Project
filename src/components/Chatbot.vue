  ...
  <div class="chatbot-container">
    ...
    <div v-if="!conversationEnded" class="input-container">
      ...
      <template v-if="currentQuestion.type === 'choice'">
        <button v-for="(answer, index) in currentQuestion.answers" :key="index" @click="handleChoice(answer)">
          {{ answer.text }}
        </button>
      </template>
      ...
      <template v-else-if="currentQuestion.type === 'nlp-input'">
        ...
        <button class="next-button" @click="submitNLPText">Analyze Symptoms</button>
      </template>

      <!-- skin upload -->
      <template v-else-if="currentQuestion.type === 'skin-upload'">
        <input type="file" ref="skinUploadInput" style="display:none" accept="image/*" @change="handleSkinUpload" />
        <button class="next-button" @click="$refs.skinUploadInput.click()">Upload Photo</button>
      </template>
    </div>
  </div>
</template>
...
const getSymptomImageUrl = (imageName) => new URL(`../assets/${imageName}`, import.meta.url).href;
...
const goToNextStep = (nextId) => {
  ...
  isTyping.value = true; userInput.value = ''; error.value = '';
  setTimeout(() => {
...
/* ---------- CHOICES / TEXT ---------- */
const handleChoice = (answer) => { addMessage(answer.text, 'user'); goToNextStep(answer.nextId); };

const submitText = () => {
  const q = currentQuestion.value;
  let valueToSubmit = userInput.value;
  if (q.inputType === 'number') {
    if (String(userInput.value).toLowerCase() === 'skip' || userInput.value === '') {
      valueToSubmit = 'Skipped';
    } else {
      const num = Number(userInput.value);
      if (isNaN(num) || (q.min != null && num < q.min) || (q.max != null && num > q.max)) {
        error.value = `Please enter a valid number between ${q.min} and ${q.max}.`; return;
      }
      valueToSubmit = num;
    }
  }
...
const toggleSymptom = (symptomLabel) => {
  ...
  if (i === -1) selectedSymptoms.value.push(symptomLabel); else selectedSymptoms.value.splice(i, 1);
};
const submitSymptoms = () => {
  const txt = selectedSymptoms.value.length ? selectedSymptoms.value.join(', ') : 'No symptoms selected.';
  addMessage(txt, 'user'); goToNextStep(currentQuestion.value.answers[0].nextId);
};

/* ---------- Skin image upload ---------- */
const handleSkinUpload = async (event) => {
  const file = event.target.files[0]; if (!file) return;
  error.value = ''; addMessage('Photo selected. Analyzing...', 'user'); isTyping.value = true;

  const formData = new FormData(); formData.append('image', file);
  try {
    const { data: result } = await axios.post(VISION_URL, formData, { headers: { 'Content-Type': 'multipart/form-data' } });
    const msg = `Analysis Result: ${result.predicted_class_name}\nConfidence: ${(result.confidence * 100).toFixed(2)}%`;
    addMessage(msg, 'bot');

    // store + jump straight to confirm
    collectedData.value.skin_analysis   = result.predicted_class_name;
    collectedData.value.skin_confidence = Math.round((result.confidence ?? 0) * 100);
    finishConversation();
  } catch (err) {
    const msg = err?.response?.data?.message || err?.message || 'Sorry, the analysis could not be completed.';
    error.value = msg; addMessage(`❌ ${msg}`, 'bot');
    goToNextStep(currentQuestion.value.answers[0].nextId);
  } finally {
    isTyping.value = false; event.target.value = '';
  }
};
...
const startRec = async () => {
  ...
  try {
    ...
    mediaRecorder.start(); isRec.value = true;
  } catch (e) {
...
.message.user { background-color: #2c5282; color: white; align-self: flex-end; }
.input-container { padding: 15px; border-top: 1px solid #e0e0e0; display: flex; flex-wrap: wrap; gap: 10px; justify-content: center; align-items: center; background: #f9f9f9; }
button { padding: 10px 15px; border: 1px solid #2c5282; background-color: white; color: #2c5282; border-radius: 20px; cursor: pointer; transition: all 0.2s; }
button:hover { background-color: #2c5282; color: white; }
.text-entry { padding: 10px 15px; border: 1px solid #ccc; border-radius: 20px; flex: 1; }
.error-message { color: #d93025; font-size: 12px; width: 100%; text-align: center; margin-top: 5px; }
.symptom-grid { display: grid; grid-template-columns: repeat(3, 1fr); gap: 10px; width: 100%; padding: 10px; }
.symptom-item { border: 2px solid #ddd; border-radius: 8px; padding: 10px; text-align: center; cursor: pointer; transition: all 0.2s ease-in-out; }
.symptom-item:hover { border-color: #2c5282; }
.symptom-item.selected { border-color: #2c5282; background-color: #e6f2ff; box-shadow: 0 0 5px rgba(44,82,130,0.5); }
.symptom-item img { width: 48px; height: 48px; margin-bottom: 5px; }
.symptom-item p { margin: 0; font-size: 12px; font-weight: bold; }
.next-button { width: calc(100% - 20px); margin-top: 10px; background-color: #2c5282; color: white; }
.nlp-textarea { width: 100%; height: 80px; border-radius: 10px; padding: 10px; border: 1px solid #ccc; font-family: sans-serif; resize: vertical; }
</style>
