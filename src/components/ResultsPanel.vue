<template>
  <div class="card">
    ...
    <template v-else>
      <header class="header">
        <h3>Final Decision: Severity {{ report.finalDecision?.severity }}</h3>
        <p class="muted">{{ report.finalDecision?.rationale }}</p>
        <small>Job: {{ report.jobId }} • {{ formatTime(report.createdAt) }}</small>
        <div class="tools">
      ...
      <section class="grid">
        ...
        <div class="block">
          <h4>Model Votes</h4>
          <table class="table">
            <thead><tr><th>Model</th><th>Severity</th><th>Confidence</th></tr></thead>
            <tbody>
...
async function copyJson(){
  try { await navigator.clipboard.writeText(JSON.stringify(props.report, null, 2)) } catch {}
}
</script>
...
.header{display:grid;grid-template-columns:1fr auto;gap:6px 12px;align-items:center}
.header h3{margin:0}
.muted{color:#6b7280}
.tools{display:flex;gap:8px}
.btn.small{padding:6px 10px;border-radius:10px;background:#111;color:#fff}
.btn.small.outline{background:#fff;color:#111;border:1px solid #111}
...
.chips li{padding:4px 8px;border:1px solid #ddd;border-radius:999px}
</style>
